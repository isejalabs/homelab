...
All three source [`lib/common.sh`](lib/common.sh) for environment validation, kubecontext construction, and unrecognized-flag handling shared across them (also sourced by several scripts above, for logging a captured subprocess's output at `debug` level).
