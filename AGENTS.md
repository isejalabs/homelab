...
- **Prerequisite items**: put anything that must happen before a PR can merge under a `## Before merging` heading in its body, as a checklist. Never assume such an item is done just because it was mentioned earlier or the user said "looks good" — before taking the action that depends on it (merging, closing the issue, applying a change), list each one back to the user explicitly and get an explicit per-item confirmation. A general go-ahead does not count as having ticked them off. This is agent-mediated only: it applies whenever an agent is asked to do the merge, not as a GitHub-side gate — merging via the UI/CLI directly bypasses it.
