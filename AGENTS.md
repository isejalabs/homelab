...
- Even a throwaway commit meant to be reverted minutes later belongs on a feature branch, never `main` — see the `track-branch` skill's `tmp(<env>): ...` convention (under Commands above) for a concrete example of a temporary, self-reverting change still living entirely on its own branch. Branch naming and the issue-first/no-PR-without-an-issue conventions are imported from commons — see `.commons/agents/branching.md`.
- Per-issue local session-state files (e.g. `.dev/{issue}_SESSION_STATE.md`, as seen in some other repos) are not adopted here — see [#1193](https://github.com/isejalabs/homelab/issues/1193) for why and for the open question of what (if anything) should replace them. (Repo-specific residual note — not generic enough for the shared `branching.md`.)
