...
- Even a throwaway commit meant to be reverted minutes later belongs on a feature branch, never `main` — see the `track-branch` skill's `tmp(<env>): ...` convention (under Commands above) for a concrete example of a temporary, self-reverting change still living entirely on its own branch.
