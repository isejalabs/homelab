...
- **Prerequisite items**: if a plan, issue, or PR description lists something as a prerequisite (e.g. "must exist/be created before merge"), never assume it's done just because it was mentioned earlier or the user said "looks good". Before taking the action that depends on it (merging, closing the issue, applying a change), list each prerequisite back to the user explicitly and get an explicit per-item confirmation — a general go-ahead does not count as having ticked them off.
- **Post-merge follow-ups**: when a PR/issue calls out items to address *after* merge, capture them as `project` memories (see the auto-memory system) so they survive across sessions and machines, and keep them updated as items get done. Note that this file is only ever read when a session starts (or re-reads it) in this repo — it can't proactively ping anyone. Actual "remind me after merge, then daily" delivery is handled by a separate scheduled task, not by this file.
