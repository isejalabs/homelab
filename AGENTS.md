...
- **Flux `Kustomization.spec.dependsOn`**: only add it when this unit's own manifests genuinely need another Flux-reconciled unit's CRDs/CRs/objects to exist first — a CRD a CR in this unit uses (e.g. `onepassword-connect`'s `ClusterSecretStore` needs `external-secrets`' CRDs, so its `flux/ks.yaml` depends on `external-secrets`), a namespace (`infra-ns`/`apps-ns`), or another object this unit's resources reference. This extends to a bootstrap-`helmfile`-installed app's own *Flux-managed* follow-on resources, not just its initial Helm install — e.g. any app whose `Service` sets the `io.cilium/lb-ipam-ips` annotation depends on `cilium` (cilium's Flux `Kustomization` applies the BGP advertisement CRs that actually announce that IP — bootstrap only installs the chart, not those CRs), and any app shipping a `SealedSecret` depends on `sealed-secrets` (needs its CRD/controller). Never add a `dependsOn` just because an app is installed via the bootstrap `helmfile` (e.g., cilium, sealed-secrets, cert-manager, external-secrets, onepassword-connect, flux-operator/instance — see `k8s/bootstrap/helmfile/apps/helmfile.yaml.gotmpl`) or has its CRDs installed via the bootstrap `helmfile` (e.g., grafana-operator, kube-prometheus-stack, snapshot-controller — see `k8s/bootstrap/helmfile/crds/helmfile.yaml.gotmpl`) with nothing in this unit's own manifests actually consuming that app's resources — the initial install itself is already done and healthy before Flux starts reconciling anything, so *that* ordering is guaranteed by the bootstrap sequence, not by Flux.
