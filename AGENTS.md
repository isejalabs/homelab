...
There is no application source code, so CI (`.github/workflows/`) is limited to static validation rather than tests. Phase 1 of [#1206](https://github.com/isejalabs/homelab/issues/1206) is complete: seven workflows now run on every PR and push to `main` — `pre-commit.yml` (the repo's `pre-commit` hooks), `kustomize-build.yml` (`kustomize build`/`kubeconform` across every `k8s/` overlay), `terragrunt-validate.yml` (`tofu fmt -check`, `terragrunt hcl format --check`, and `terragrunt validate` across every `terragrunt/` unit), `helmfile-template.yml` (renders `k8s/bootstrap/helmfile/{crds,apps}` to catch `.gotmpl` errors), `renovate-config-validate.yml` (`renovate-config-validator` against `.github/renovate.json5` and its includes), `check-sorting.yml` (leading-field/metadata ordering conformance with `.agents/instructions/sorting.md` — partial: that ordering only, not full list-sorting), and `actionlint.yml` (lints the workflow files themselves). None is wired up as a GitHub branch-protection required check yet. Phase 2 — checks that need a live/ephemeral cluster (`flux-local`, `kind`/`k3d`, policy checks) — is tracked in [#1209](https://github.com/isejalabs/homelab/issues/1209). Anything not yet covered is still validated manually via `helmfile template` (used implicitly by `just bootstrap::cluster`) and Flux's own reconciliation status.
...
- **No manual line breaks within a Markdown paragraph** — whether a repo `.md` file (docs, READMEs) or a GitHub issue/PR body. Write each paragraph as a single line and let the editor/terminal/renderer soft-wrap it for display; only break the line for an actual paragraph break (blank line), a list item, a heading, or a code block. A hard-wrapped paragraph renders identically on GitHub (a single `\n` collapses to a space) but makes diffs noisy — editing one word near a wrap point reflows and diffs the whole paragraph instead of just that line.
- **Never backslash-escape backticks (or other Markdown punctuation) in a GitHub issue/PR/comment body.** Post the body as plain Markdown text exactly as it should render — an inline code span is a literal `` `foo` ``, never `` \`foo\` ``. This has slipped in before ([#1275](https://github.com/isejalabs/homelab/issues/1275)): a comment drafted or previewed via a Bash heredoc, where backticks must be escaped to stop the shell from treating them as command substitution, then got posted through the GitHub MCP tools (`add_issue_comment`/`create_pull_request`/etc.) with that shell-escaping still attached, so GitHub rendered a literal backslash instead of a code span. Those tool parameters are plain strings with no shell involved — draft the text directly, or if you do stage it in a shell heredoc first, strip the escaping back out before it goes into the tool call.
  It recurred a second time ([#1278](https://github.com/isejalabs/homelab/issues/1278), [#1313](https://github.com/isejalabs/homelab/issues/1313), [#1318](https://github.com/isejalabs/homelab/issues/1318), [#1322](https://github.com/isejalabs/homelab/issues/1322), [#1330](https://github.com/isejalabs/homelab/issues/1330)) specifically for `gh issue create`/`gh issue edit`, never for `gh pr create` — coding agents are typically given a correct, unescaped worked example for PR creation but none for issues, so issue bodies get drafted ad hoc and inconsistently over-escaped. Use the same pattern for issues as for PRs: a single-quoted `'EOF'` heredoc delimiter (`gh issue create --body "$(cat <<'EOF' ... EOF )"`) means the shell won't touch `` ` `` or `$` inside it, so inline code spans and fenced code blocks go in verbatim, unescaped.
- **Branch naming and the issue-first prerequisite for any new branch** — see `.agents/instructions/branching.md`.
- **Always format references to GitHub issues, PRs, and CI jobs/runs as markdown links**, not bare numbers or plain text — e.g. `[#1234](https://github.com/isejalabs/homelab/issues/1234)`, or `[owner/repo#42](https://github.com/owner/repo/issues/42)` for a cross-repo reference — wherever one appears in a chat reply, PR/issue body, or comment, so it's directly clickable. Exception: a closing keyword in a commit message or PR body (`Closes #123`, `Fixes #123`, `Relates to #123`) stays as the bare reference, since GitHub only auto-links/auto-closes on that exact form.
...
- **CI exists but isn't a hard gate.** Seven GitHub Actions workflows (`pre-commit`, `kustomize-build`, `terragrunt-validate`, `helmfile-template`, `renovate-config-validate`, `check-sorting`, `actionlint`) run on every PR (see "Commands" above). There's no branch protection requiring them to pass, so GitHub won't block a merge on red checks — but treat them as if it did: check `gh pr checks <n>` and don't call a PR mergeable while any check is failing or still running. These are fast (well under a minute each), so it's fine to wait for them rather than merging blind.
- **Post-merge follow-ups**: put anything to address *after* merge (not blocking merge) under a `## Post-merge follow-ups` heading in the PR body, as a checklist — kept separate from `## Before merging` above. Don't pre-create a tracking issue for this speculatively before the PR exists or merges (that caused duplicate/stale issues in the past, e.g. [#1117](https://github.com/isejalabs/homelab/issues/1117) vs [#1118](https://github.com/isejalabs/homelab/issues/1118), and again [#1157](https://github.com/isejalabs/homelab/issues/1157)/[#1158](https://github.com/isejalabs/homelab/issues/1158)/[#1159](https://github.com/isejalabs/homelab/issues/1159)/[#1161](https://github.com/isejalabs/homelab/issues/1161) — see [#1203](https://github.com/isejalabs/homelab/issues/1203)). Instead, once the PR merges, automation reads that heading and opens exactly one tracking issue labeled `post-merge-followup`, linked back to the PR; a separate daily digest emails every open issue carrying that label until it's closed. That label is reserved for this automation — never apply it by hand to a regular issue, so it stays a small, clean queue distinct from the normal issue backlog/project board. Most PRs won't need this section at all. This automation is external (Claude Code app-side, not repo-local — there's no `.github/` config surface for it), and it has misfired into duplicates twice now: before creating a `post-merge-followup` issue by hand for a case it missed, or acting on one it just created, search open *and closed* issues for that label referencing the same PR number; if more than one already exists for the same PR, close all but the oldest as a duplicate of it (`duplicate_of`) rather than leaving several open.
