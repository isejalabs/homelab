...
- **Recording a "blocked by" relationship**: when one issue genuinely can't be worked/closed until another issue lands, set it up as a real GitHub issue relationship via `gh api graphql` (`addBlockedBy`, taking `issueId`/`blockingIssueId` node IDs from an `issue(number: N) { id }` query) — this shows up as a native tracked relationship, not just prose. This mutation only accepts Issue nodes on both sides, though — it rejects a PR's node ID outright. When the blocked (or blocking) side is a PR rather than an issue, fall back to the `## Before merging` checklist convention above instead; there's no native link for that shape yet.
...
- **Linking to another repo's issue/PR** (upstream dependency, third-party provider, etc.) from one of ours: use a short link through `redirect.github.com` instead of a plain `github.com` link, e.g. `[owner/repo#123](https://redirect.github.com/owner/repo/pull/123)` (or `.../issues/123`). A plain link creates a "mentioned this pull request" cross-reference on the *target's* timeline, which is noise on a repo we don't own; the `redirect.github.com` form still resolves to the same page for readers but doesn't trigger that notification. This is a global convention (also in `~/.claude/CLAUDE.md`), not specific to this repo.
