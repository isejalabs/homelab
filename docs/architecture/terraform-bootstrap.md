...
- On top of S3's own `encrypt = true` (SSE), Terragrunt additionally encrypts state client-side with `pbkdf2`
  using `state_encryption_passphrase` — a SOPS secret (see
  [`secrets.md`](secrets.md#sops-provisioning-secrets)). This is the detail that makes the bootstrap order
  matter: decrypting that passphrase is a prerequisite for touching state at all, so the age key used for
  SOPS has to exist before the very first `terragrunt init`.
