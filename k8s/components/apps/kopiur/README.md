...
- **`secret/`** — a `kopiur-repository-secret` `ExternalSecret`, namespaced wherever it's included. The mover Job that talks to `fiona` needs these credentials in its _own_ namespace (`envFrom` is namespace-local), so every consumer includes this component rather than referencing a single shared `Secret` — `kopiur-repository` itself (in `kopiur-system`) includes it the same way `apps/storage/pvc`/`pvc-no-backup` do.
