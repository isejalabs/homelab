...
locals {
  ...
  # in the merge below). Shape matches the `proxmox`/`proxmox_api_token` variables of
  # isejalabs/terraform-proxmox-talos (base_source_url above).
  secret_defaults = {
    proxmox = {
      cluster_name = "ci-placeholder"
      endpoint     = "https://ci-placeholder:8006"
      insecure     = true
      username     = "ci-placeholder"
    }
    proxmox_api_token = "ci-placeholder"
  }
  ...
  base_source_url = "git::https://github.com/isejalabs/terraform-proxmox-talos.git"

  # Set some values common accross all environments
  cilium_path           = "k8s/infra/kube-system/cilium"
